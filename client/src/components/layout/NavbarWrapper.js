...
export default function NavbarWrapper()  {
	...
	const navLogout = () => {
 	...
 	return (
		<Navbar className='navbar-container h-100'  variant='dark' expand="sm" >
						...
						<Nav>
							{!isUserLoading &&
								<Container fluid className='d-flex justify-content-end align-items-center'>
									...
									<NavDropdown className='' title={user ? user.displayName + ' ' : 'User '} id="basic-nav-dropdown" menuVariant='dark'>
										<Link to="/settings" className={`nav-link drop-down ms-1 ${(user) ? '' : 'disabled-link'}`} onClick={() => updateMessages({})}>Settings</Link>
										<Link to="/" className='nav-link drop-down ms-1' onClick={navLogout}>Log out</Link>
									</NavDropdown>
								</Container>
							}
						</Nav>
